...
class TorchTD3Optimizer(TorchOptimizer):
    ...
    def q_loss(
        ...
        target_q1_out: Dict[str, torch.Tensor],
        target_q2_out: Dict[str, torch.Tensor],
        dones: torch.Tensor,
        ...
        for i, name in enumerate(q1_out.keys()):
            ...
            with torch.no_grad():
                ...
                _dones = dones.unsqueeze(-1) if dones.dim() == 1 else dones
                _rewards = rewards[name].unsqueeze(-1) if rewards[name].dim() == 1 else rewards[name]

                # Min Target Q
                min_target_q = torch.min(target_q1_out[name], target_q2_out[name])
                
                q_backup = _rewards + (
                    (1.0 - self.use_dones_in_backup[name] * _dones)
                    * self.gammas[i]
                    * min_target_q
                )
    ...
    def update(self, batch: AgentBuffer, num_sequences: int) -> Dict[str, float]:
        if self.update_counter == 0:
            logger.info(f"TD3 Optimizer using device: {default_device()}")
        ...
        with torch.no_grad():
            ...
            noise = torch.randn_like(next_action.continuous_tensor) * self.target_noise
        ...
        q1_loss, q2_loss = self.q_loss(
            q1_out, q2_out, target_q1, target_q2, dones, rewards, masks
        )
